...
export const validateChangeInfo = (fields: any) => {
  const errors: IErrors = {
    name: "",
    phone: "",
  };
  const validateName = /^[A-Z][a-zA-Z\s-]*$/;
  const validatePhone = /^[0-9\-\(\)\s]+$/;

  if (!fields.name.trim() || !validateName.test(fields.name)) {
    errors.name =
      "The name must be between 2 and 50 characters long and can only contain letters, hyphens, and spaces. It must start with a capital letter.";
  }
  if (!fields.phone.trim() || !validatePhone.test(fields.phone)) {
    errors.phone =
      "The phone number must be in a valid format and contain at least 9 digits.";
  }
  return errors;
