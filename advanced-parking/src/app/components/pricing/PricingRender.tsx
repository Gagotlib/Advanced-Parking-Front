...
function PricingRender({ duration, setFormData }: PricingRenderProps) {
  ...
  const [selectedHour, setSelectedHour] = useState<number | null>(1);
    ...
    <div className="w-[320px] overflow-x-scroll scroll  whitespace-nowrap scroll-smooth focus:scroll-auto">
      <div className='flex p-1 gap-2 text-xs'>
        {durations.map((hour: any, index: any) => (
          <div key={index}>
            <div
              className="flex flex-col p-2 items-center border border-silver/80 shadow-sm shadow-erieblack/80 rounded-lg hover:scale-105 "
            >
